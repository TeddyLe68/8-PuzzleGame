...
class PuzzlePage(tk.Frame):
    ...
    def run_solution(self):
        ...
        path_to_goal, nodes_expanded, max_search_depth, time_elasped = Board.solve(
            self.current_board_state, self.algorithm.func
        )

        if not self.is_stopped:
            print(
                f"Done in {round(time_elasped, 4)} second(s) with {len(path_to_goal)} moves using {self.algorithm.name}"
            )
            print(
    ...
    def transform_click(self, tile_index):
        ...
        for action in possible_actions:
            if not self.is_solving and not self.is_done:
                if (
                    action == "U"
                    and self.current_board_state[blank_index - 3] == tile_value
                ):
                    ...
                    action == "L"
                    and self.current_board_state[blank_index - 1] == tile_value
                ):
                    self.transform_state(action)
